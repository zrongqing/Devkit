...
using Barcode2.Core.Attributes;
...
public class RoslynApiScanner : IApiScanner
{
    ...
    /// <inheritdoc />
    public string GetExecutionSourceCodeByName(string sourcePath, string apiName)
    {
        if (string.IsNullOrWhiteSpace(sourcePath) ||
        ...
        lock (_cacheSync)
        {
            var files = GetSourceFiles(sourcePath, "*.cs");
            if (TrySynchronizeCache(sourcePath, "*.cs", files) &&
                _cache.TryGetExecutionSourcesByName(sourcePath, "*.cs", apiName, out var cachedSources))
            {
                return JoinExecutionSources(cachedSources);
            }
            ...
            var analyses = AnalyzeFilesWithoutCache(files);
            var sources = analyses
                .SelectMany(analysis => analysis.ExecutionSourcesByName.TryGetValue(apiName, out var matches)
                                            ? matches
                                            : [])
                .ToList();
            return JoinExecutionSources(sources);
    ...
    /// <inheritdoc />
    public List<ApiSourceInfo> GetApiSourceInfosByName(string sourcePath, string apiName)
    {
        if (string.IsNullOrWhiteSpace(sourcePath) ||
            string.IsNullOrWhiteSpace(apiName) ||
            !Directory.Exists(sourcePath))
        ...
        lock (_cacheSync)
        {
            ...
            var files = GetSourceFiles(sourcePath, searchPattern);
            if (TrySynchronizeCache(sourcePath, searchPattern, files) &&
                _cache.TryGetApiSourcesByName(sourcePath, searchPattern, apiName, out var cachedSources))
            {
                return cachedSources;
            }
    ...
    private SourceFileAnalysis ParseSourceFile(string filePath, string sourceCode)
    {
        ...
        foreach (var classDecl in classDeclarations)
        {
            ...
            if (apiAttributes.Count > 0)
            {
                ...
                foreach (var apiAttr in apiAttributes)
                {
                    ...
                    {
                        var apiName = GetApiNameValue(apiAttr);
        ...
        var executionSourcesByName = new Dictionary<string, List<string>>(StringComparer.Ordinal);
        ...
        foreach (var method in methods)
        {
            ...
            var apiAttributes = method.AttributeLists
                .SelectMany(attributeList => attributeList.Attributes)
                .Where(attribute => ExecutionSourceAttributes.Contains(attribute.Name.ToString()))
                .ToList();

            foreach (var attribute in apiAttributes)
            {
                var apiCode = GetApiCodeValue(attribute);
                if (!string.IsNullOrWhiteSpace(apiCode))
                {
                    AddExecutionSource(executionSources, apiCode, methodSource);
                    continue;
                }

                var apiName = GetApiNameValue(attribute);
                if (!string.IsNullOrWhiteSpace(apiName))
                {
                    AddExecutionSource(executionSourcesByName, apiName, methodSource);
                }
            }
        }
        ...
        return new SourceFileAnalysis
        {
            ...
            ExecutionSourcesByName = executionSourcesByName
        };
    }
    ...
    {
        var parserKey = $"3|{string.Join('|', Attributes.OrderBy(attribute => attribute, StringComparer.Ordinal))}";
    ...
    {
        var positionalArgument = attr.ArgumentList?.Arguments
            .FirstOrDefault(argument =>
                argument.NameEquals is null &&
                (argument.NameColon is null ||
                 argument.NameColon.Name.Identifier.ValueText == "code"));
        return positionalArgument?.Expression.ToString().Trim('"') ?? string.Empty;
    }

    private static string GetApiNameValue(AttributeSyntax attr)
    {
        var nameArgument = attr.ArgumentList?.Arguments
            .FirstOrDefault(argument => argument.NameEquals?.Name.Identifier.ValueText == nameof(ApiExtendCodeAttribute.Name));
        return nameArgument?.Expression.ToString().Trim('"') ?? string.Empty;
    }

    private static void AddExecutionSource(
        IDictionary<string, List<string>> executionSources,
        string identifier,
        string source)
    {
        if (!executionSources.TryGetValue(identifier, out var sources))
        {
            sources = [];
            executionSources.Add(identifier, sources);
        }
