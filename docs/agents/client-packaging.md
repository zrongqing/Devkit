...
- 指向 `main` 已合入提交、并与 `VersionPrefix` 一致的正式版本标签。
...
- GitHub Release 中的正式安装器、SHA-256 文件和自动生成的发布说明。
...
4. 以 `win-x64`、框架依赖方式发布主程序，并汇总 Demo、ModuleName、Barcode2 三个动态模块。
5. 验证 staging 的主程序、配置和模块结构，移除 PDB、测试程序集和中间文件。
6. 使用 Inno Setup 生成当前用户安装器并生成 SHA-256。
7. 持续集成仅在所有步骤成功后上传 Artifact；不自动创建 Release。
8. 正式发布仅接受 `main` 上与 `VersionPrefix` 一致的 `v<version>` 标签，并在完整打包成功后创建 GitHub Release。
...
- 正式标签使用 `v<VersionPrefix>`；标签版本、安装器版本与源码基线版本必须一致。

## 安全与边界

- 不提交安装包、staging、PDB、测试输出或其他构建产物。
- 不提交代码签名证书、私钥、密码、令牌、Syncfusion 许可证、真实连接串或本机配置。
- 正式发布工作流仅使用内置 `GITHUB_TOKEN`，只授予创建 Release 所需的 `contents: write`；持续集成打包保持 `contents: read`。
- 当前不自动下载运行时、不签名，也不扩展 ARM64/MSIX/自动更新。
...
- 正式发布配置缺失或无效：拒绝格式错误、版本不匹配或不属于 `main` 的标签，不创建 Release。
- GitHub Release 创建失败：工作流返回失败，不用 Actions Artifact 冒充正式发布。
...
- 静默安装、文件检查和静默卸载通过。
- `main` push 与 `workflow_dispatch` 均调用同一脚本，持续集成权限保持 `contents: read`。
- 正式版本标签调用同一脚本，并只在完整打包成功后创建带安装器和校验文件的 GitHub Release。
