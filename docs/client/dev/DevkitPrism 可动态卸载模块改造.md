...
- 从 `Module.Build.props` 移除当前对所有模块无差别添加的业务依赖；Demo、ModuleName、Ssamc 分别声明自己直接使用的包。
- Ssamc 目录仅保留入口 DLL、`.deps.json`、私有依赖及其本机/资源文件，例如 HandyControl、Roslyn、EF Core、Oracle；Demo 和 ModuleName 不再携带 Ssamc 的重型依赖。
...
- Demo、ModuleName、Ssamc 在初始化时为全部菜单标记模块 ID，卸载时注销本模块菜单和其他显式资源。
...
- Ssamc 的 SQLite 源码缓存关闭连接池，避免操作完成或模块卸载后继续占用数据库文件；这也修复当前 3 个 `source-cache.db` 基线测试失败。

## Test Plan

- 清理后构建解决方案，验证三个模块目录与主程序根目录的 DLL 文件名交集为空，且 Ssamc 私有托管、本机和资源依赖仍完整。
